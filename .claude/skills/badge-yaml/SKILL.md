...
reference_pages:                            # OPTIONAL — only include when the badge
  - title: ...                              # cites a standing reference
    body: shared/<file>.md                  # (e.g., LNT, the Outdoor Code).
```
...
| `checkbox`       | One attestation checkbox. With a `label:` it shows that text. With **no label** it's a *confirmation* checkbox — it renders a muted "Completed" box, in line with the prompt, for requirements where the Scout just demonstrates/does the task (e.g. "demonstrate how to force checkmate"). Prefer the label-less form for pure "I did this" items. |
